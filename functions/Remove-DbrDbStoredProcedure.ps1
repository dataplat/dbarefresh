        ...
        if ($totalObjects -ge 1) {
            if ($PSCmdlet.ShouldProcess("Removing stored procedures in database $Database")) {
                ...
                foreach ($object in $procedures) {
                    $objectStep++
                    $operation = "Procedure [$($object.SchemaName)].[$($object.Name)]"
                    ...
                    Write-PSFMessage -Level Verbose -Message "Dropping stored procedure [$($object.SchemaName)].[$($object.Name)] from $Database"
