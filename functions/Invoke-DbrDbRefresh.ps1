                ...
                if (-not $SkipTable) {
                    ...
                    if (-not $SkipData) {
                        ...
                        foreach ($itemTable in $item.Tables) {
                            ...
                            $sourceTable = $sourceDb.Tables | Where-Object { $_.Schema -eq $itemTable.Schema -and $_.Name -eq $itemTable.Name }
                            ...
                            if ($PSCmdlet.ShouldProcess("$($destServer)", "Creating table(s) and copying data")) {
                                ...
                                $bulkCopy = New-Object Data.SqlClient.SqlBulkCopy("$connstring;Database=$($destDb.Name)", $bulkCopyOptions)

                                $columnsToCopy = $itemTable.Columns | Where-Object { $_.IsComputed -eq $false -and $_.IsGenerated -eq $false }

                                foreach ($itemTableColumn in $columnsToCopy) {
                                ...
                                if ($Pscmdlet.ShouldProcess($destServer, "Writing rows to [$($itemTable.Schema)].[$($itemTable.Name)]")) {
                                    try {
                                        ...
                                        Stop-PSFFunction -Message "Something went wrong executing the reader" -Target $reader -ErrorRecord $_ -Continue
                                        return
                                    }
                                    ...
                                    try {
                                        ...
                                        Stop-PSFFunction -Message "Something went wrong writing [$($itemTable.Schema)].[$($itemTable.Name)]" -Target $destServer -ErrorRecord $_ -Continue
                                        return
                                    }
                            ...
                            Write-PSFMessage -Level Verbose -Message "Row Count Source:         $($sourceRowCount.RowCount)"
                            ...
                            [PSCustomObject]@{
                                ...
                                Notes                    = "Copied $($sourceRowCount) of $($rowCountSource) rows"
